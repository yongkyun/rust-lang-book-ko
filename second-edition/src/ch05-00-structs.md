...
구조체는 3장에서 논의했던 tuple과 비슷합니다. tuple처럼 하나의 구조체의 한 조각은 다른 유형이 될 수 
있습니다. tuple과 달리 우리는 각 데이터에 이름을 붙일 수 있어서 값이 어떤 의미를 가지는지 명화하게 
나타낼 수 있습니다. 이러한 이름을 통해 구조체는 tuple보다 더 복잡합니다: 우리는 인스턴스의 값을 
지정하거나 액세스하기 위해 데이터의 순서에 의존할 필요가 없습니다.

구조체를 정의하기 위해 우리는 `struct`라른 키워드를 입력하고 전체 구조체의 이름을 정의합니다. 구조체의 
이름은 함께 묶여있는 데이터들의 의미를 나타입니다. 그리고나서 중간호 안에 데이터들의 이름과 형식을 
정의하며, 필드(*fields*)라고 부릅니다. 예를 들어, Listing 5-1은 사용자 계정에 대한 정보를
정의하기 위한 구조체를 나타냅니다.
...
우리가 이것을 정의한 후에 구조체를 사용하기 위해서 우리는 각 필드를 위한 구체적인 값을 정의함으로써 
구조체의 인스턴스(*instance*)를 만들어야 하고, 그리고 나서 키는 필드의 이름이고 값은 우리가 필드에 
저장하고 싶은 데이터를 나타내는 `키(key):값(value)` 쌍을 가진 중가로를 추가합니다. 우리는 구조체에서 
정의한 순서대로 필드를 정의할 필요는 없습니다. 다른 말로, 구조체 정의는 특정 형식을 위한 일반적인 
템플릿(template)과 같고, 인스턴스는 특정 형식의  데이터를 만들기 위해 특정한 데이터를 가지고 템플릿을 
채웁니다. 예를 들어, 우리는 Listing 5-2과 같이 특정한 사용자를 정의할 수 있습니다:
